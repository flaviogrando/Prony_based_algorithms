...
function [Ha, Hb, Hc, Apa, Apb, Apc, phipa, phipb, phipc] = estimador_red_prony_3(Va_seg, Vb_seg, Vc_seg, freq_final, Fs, ref_seg, amort_final)
...
for i=1:num_wins  % inicia a partir da 2nd janela devida a med de 
    ...
    %--- step - reduced prony ---
%     re_z = exp(alfa(i)*T);
%     im_z = exp(1i*2*pi*f*T);
    re_z = exp(alfa(i)*T);
    im_z = tan(2*pi*f*T)*re_z;
