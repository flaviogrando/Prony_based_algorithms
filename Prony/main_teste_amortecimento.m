...
amort_ref = ones(1,2)*amort(w);   % Vetor de referência para amortecimento
...
T = 0.04;        % tempo total (em segundos)
...
% 
% figure, hold on, grid on
% plot((1-exp(amort_final_ref*1/Fs))*100, 'o-')
...
% COM DADOS FFT
[Apa4, Apb4, Apc4, phipa4, phipb4, phipc4] = estimador_mod_red_prony(Va_seg, Vb_seg, Vc_seg, ref_seg(1,:), Fs, ref_seg, amort_final_fft);
...
amp_med(6,i:f) = Apa4(i:f,1)'; % com freq ref e amort (amp de dft)
...
% 
i=1;  % inicio
f=61; % fim (numero de ciclos)

% Plot ERRO de fasores (DFT e Prony)
figure  
subplot(3,1,1), hold on, grid on%
plot(amort(i:f), amort_tve(1,:), '*-')
plot(amort(i:f), amort_tve(2,:), 'o-')
%plot(amort(i:f), tve(3,:), 's-')
plot(amort(i:f), amort_tve(4,:), 'd-') 
plot(amort(i:f), amort_tve(5,:), 'x-') % 
plot(amort(i:f), amort_tve(6,:), '+-') % 
plot(amort(i:f), amort_tve(7,:), 's-') % 
ylabel('TVE (%)'), xlabel('fator de amortecimento (\alpha)')
%title('TVE (%)') 
%legend('DFT', 'R-Prony (ref)', 'R-Prony (dft)', 'MR-Prony (ref)', 'MR-Prony (dft)')
%title('TVE (%)'), legend('R-Prony (ref)', 'R-Prony (dft)', 'MR-Prony (ref)', 'MR-Prony (dft)') 
subplot(3,1,2), hold on, grid on
plot(amort(i:f), amort_amp_error(1,:), '*-')
plot(amort(i:f), amort_amp_error(2,:), 'o-')
%plot(amp_error(3,:), 's-')
plot(amort(i:f), amort_amp_error(4,:), 'd-')
plot(amort(i:f), amort_amp_error(5,:), 'x-')
plot(amort(i:f), amort_amp_error(6,:), '+-')
plot(amort(i:f), amort_amp_error(7,:), 's-')
%title('Erro de amplitude (%)')
%legend('DFT', 'R-Prony (ref)', 'R-Prony (dft)', 'MR-Prony (ref)', 'MR-Prony (dft)')
%title('Erro de amplitude (%)'), legend('R-Prony (ref)', 'R-Prony (dft)', 'MR-Prony (ref)', 'MR-Prony (dft)') 
ylabel('Erro de amplitude (%)'), xlabel('fator de amortecimento (\alpha)')
subplot(3,1,3), hold on, grid on
plot(amort(i:f), amort_phase_error(1,:), '*-')
plot(amort(i:f), amort_phase_error(2,:), 'o-')
%plot(amort(i:f), amort_phase_error(3,:), 's-')
plot(amort(i:f), amort_phase_error(4,:), 'd-')
plot(amort(i:f), amort_phase_error(5,:), 'x-')
plot(amort(i:f), amort_phase_error(6,:), '+-')
plot(amort(i:f), amort_phase_error(7,:), 's-')
%title('Erro de fase (º)')
...
for i=1:length(tve(:,1))
   max_tve(i) = max(amort_tve(i,:));
   max_amp_error(i) = max(amort_amp_error(i,:)); 
   max_phi_error(i) = max(amort_phase_error(i,:)); 
end
%