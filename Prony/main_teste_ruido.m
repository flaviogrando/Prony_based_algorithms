...
noise = ruido(w);    % nível de ruído em dB ( 0 = sem ruído)
T = 4;        % tempo total (em segundos)
...
figure
subplot(3,1,1), hold on, grid on%
boxplot(tve_dft, ruido)
xlabel('SNR (dB)'), ylabel('TVE (%)'), legend('DFT')
subplot(3,1,2), hold on, grid on%
boxplot(tve_r_prony, ruido)
xlabel('SNR (dB)'), ylabel('TVE (%)'), legend('R-Prony')
subplot(3,1,3), hold on, grid on%
boxplot(tve_mr_prony, ruido)
xlabel('SNR (dB)'), ylabel('TVE (%)'), legend('RM-Prony')
...
ylabel('TVE (%)')
title(['SNR = ', num2str(ruido(1))])
subplot(2,2,2), hold on, grid on
boxplot(b, 'Labels',{'a','b', 'c','d', 'e'})
ylabel('TVE (%)')
title(['SNR = ', num2str(ruido(3))])
subplot(2,2,3), hold on, grid on
boxplot(c, 'Labels',{'a','b', 'c','d', 'e'})
ylabel('TVE (%)')
title(['SNR = ', num2str(ruido(5))])
subplot(2,2,4), hold on, grid on
boxplot(d, 'Labels',{'a','b', 'c','d', 'e'})
ylabel('TVE (%)')
title(['SNR = ', num2str(ruido(7))])